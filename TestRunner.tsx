...
export default function TestRunner({test}:{test:any}){
 ...
 const [comp,setComp]=useState<string[]>(Array(test.questions.length).fill(""));
 ...
 const essayOptions=String(test.essayPrompt||"").split("||").map((x:string)=>x.trim()).filter(Boolean);
 ...
 async function save(){if(!attemptId)return;await fetch(`/api/attempts/${attemptId}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({essayAnswer:essay,compAnswers:comp})});}
 async function submit(auto=false){
  ...
  setSubmitted(true); if(auto)alert("Time is over. Your test has been auto-submitted."); else setShow(false);
 }
 ...
 return <div className="examTop" style={{minHeight:"100vh",display:"block",padding:0}}>
  ...
  <div className="examLayout"><div className="main"><div className="qtitle">Descriptive Test No {test.testNo}</div>
   ...
   <div className="body">{section==="essay"?<>
    <h3>Question No 1 — Essay Writing (15 Marks)</h3>
    <p><b>Write an essay on ANY ONE of the following topics:</b></p>
    <div style={{display:"grid",gap:12,margin:"16px 0 20px"}}>
     {(essayOptions.length?essayOptions:[String(test.essayPrompt||"")]).map((topic:string,i:number)=><label key={i} style={{display:"flex",gap:12,alignItems:"flex-start",padding:15,border:"1px solid #cbd3df",borderRadius:10,background:selectedEssay===topic?"#eef5ff":"#fff",cursor:"pointer"}}>
      <input type="radio" name="essayOption" value={topic} checked={selectedEssay===topic} onChange={()=>setSelectedEssay(topic)} style={{marginTop:4}} />
      <span><b>Option {String.fromCharCode(65+i)}:</b> {topic}</span>
     </label>)}
    </div>
    <p style={{fontSize:13,color:"#5d687b"}}>Select one option and write your essay in <b>250–300 words</b>.</p>
    <textarea className="answer" value={essay} disabled={!selectedEssay} placeholder={selectedEssay?"Start writing your essay here...":"Select an essay option above to start writing"} onChange={e=>setEssay(e.target.value)} />
    <div className="counter">Words: {count(essay)} / 250–300</div>
   </>:<>
    <h3>Question No 2 — Comprehension (10 Marks)</h3>
    <div className="card"><b>Directions:</b> Read the following passage carefully and answer the questions in your own words.<p style={{whiteSpace:"pre-line",lineHeight:1.55}}>{test.passage}</p></div>
    {test.questions.map((q:any,i:number)=><div key={q.id} style={{margin:"20px 0"}}><p><b>Q{i+1}. {q.prompt}</b></p><textarea className="answer" style={{minHeight:120}} value={comp[i]||""} onChange={e=>{const x=[...comp];x[i]=e.target.value;setComp(x)}}/><div className="counter">Words: {count(comp[i]||"")} / 40</div></div>)}
   </>}</div>
   <div className="footer"><button className="btn outline" onClick={save}>Save Response</button><div><button className="btn outline" onClick={()=>setSection(section==="essay"?"comp":"essay")}>Save & Next</button><button className="btn primary" onClick={()=>setShow(true)} style={{marginLeft:8}}>Submit</button></div></div>
  </div>
