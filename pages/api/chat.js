  ...
  try{
    ...
    if(req.method==="GET"){
      if(admin){
        ...
        const r=await db.query(`SELECT a.phone,MAX(a.name) AS name,MAX(m.created_at) AS last_message,COUNT(m.id)::int AS message_count FROM dm_access_requests a LEFT JOIN dm_chat_messages m ON m.phone=a.phone WHERE a.status='approved' GROUP BY a.phone ORDER BY last_message DESC NULLS LAST, a.phone ASC LIMIT 100`);
