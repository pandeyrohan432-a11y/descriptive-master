import { SBI_CLERK_2025 } from '../../data/pre_mock_data';
...
export default async function handler(req,res){
  ...
  try{
    const data=SBI_CLERK_2025;
    if(!Array.isArray(data)||data.length!==100){
      throw new Error(`Invalid mock data: expected 100 questions, got ${Array.isArray(data)?data.length:'non-array'}`);
    }
