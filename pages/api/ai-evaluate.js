...
function normalise(result) {
  ...
  return {
    essayScore: Number(essayScore.toFixed(1)), comprehensionScore: Number(comprehensionScore.toFixed(1)), totalScore: Number((essayScore + comprehensionScore).toFixed(1)),
    essay: { score: Number(essayScore.toFixed(1)), breakdown: { relevance: clamp(essay.breakdown?.relevance, 0, 4), structure: clamp(essay.breakdown?.structure, 0, 3), arguments: clamp(essay.breakdown?.arguments, 0, 3), grammar: clamp(essay.breakdown?.grammar, 0, 3), vocabulary: clamp(essay.breakdown?.vocabulary, 0, 2) }, whatWasGood: Array.isArray(essay.whatWasGood) ? essay.whatWasGood : [], whatWasWrong: Array.isArray(essay.whatWasWrong) ? essay.whatWasWrong : [], improvements: Array.isArray(essay.improvements) ? essay.improvements : [], modelAnswer: essay.modelAnswer || "" },
...
const responseSchema = { type: "OBJECT", properties: {
  essay: { type: "OBJECT", properties: { score: { type: "NUMBER" }, breakdown: { type: "OBJECT", properties: { relevance: { type: "NUMBER" }, structure: { type: "NUMBER" }, arguments: { type: "NUMBER" }, grammar: { type: "NUMBER" }, vocabulary: { type: "NUMBER" } }, required: ["relevance", "structure", "arguments", "grammar", "vocabulary"] }, whatWasGood: { type: "ARRAY", items: { type: "STRING" } }, whatWasWrong: { type: "ARRAY", items: { type: "STRING" } }, improvements: { type: "ARRAY", items: { type: "STRING" } }, modelAnswer: { type: "STRING" } }, required: ["score", "breakdown", "whatWasGood", "whatWasWrong", "improvements", "modelAnswer"] },
