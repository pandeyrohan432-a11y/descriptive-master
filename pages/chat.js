...
export default function Chat(){
  ...
  return <div style={styles.page}>
    ...
    <div style={styles.wrap}><div style={styles.card}>
      <div style={styles.head}><div><b>👥 Student Community</b><span style={styles.sub}> • All logged-in students</span></div><button onClick={enableNotifications} style={styles.notify}>{notif==="granted"?"🔔 Notifications On":"🔔 Enable Notifications"}</button></div>
      {notice&&<div style={styles.notice}>🔔 {notice}</div>}
      <div style={styles.messages}>
        {messages.length===0?<div style={styles.empty}>No messages yet. Start the conversation.</div>:messages.map(function(m){return <div key={m.id} style={{display:"flex",justifyContent:m.sender==="student"?"flex-end":"flex-start"}}><div style={{...styles.bubble,background:m.sender==="student"?"#dcecff":"#fff"}}><div style={styles.sender}>{m.sender==="admin"?"Admin":(m.name||"Student")}</div><div>{m.message}</div><div style={styles.time}>{new Date(m.created_at).toLocaleString("en-IN")}</div></div></div>;})}
        <div ref={bottom}/>
      </div>
      {error&&<div style={styles.error}>{error}</div>}
      <div style={styles.composer}>{chatBlocked?<div style={styles.blocked}>🚫 You have been blocked from the student group chat by the admin.</div>:<><input style={styles.input} value={text} onChange={function(e){setText(e.target.value.slice(0,2000));}} onKeyDown={function(e){if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();send();}}} placeholder="Message the student group..."/><button style={styles.button} disabled={busy} onClick={send}>{busy?"Sending...":"Send"}</button></>}</div>
    </div></div>
  </div>;

const styles={tab:{border:"1px solid #cbd3df",background:"#fff",borderRadius:8,padding:"7px 10px",cursor:"pointer",fontSize:12},tabOn:{background:"#40537d",color:"#fff",borderColor:"#40537d"},page:{minHeight:"100vh",background:"#f5f7fb",fontFamily:"Arial,Helvetica,sans-serif",color:"#172033"},top:{height:64,background:"#40537d",color:"#fff",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 20px"},link:{color:"#fff",textDecoration:"none"},wrap:{maxWidth:850,margin:"0 auto",padding:20},card:{background:"#fff",border:"1px solid #d7dee9",borderRadius:14,overflow:"hidden",boxShadow:"0 5px 20px rgba(25,45,80,.08)"},head:{padding:16,borderBottom:"1px solid #e3e7ee",fontWeight:700,display:"flex",justifyContent:"space-between",alignItems:"center",gap:12},sub:{fontWeight:400,color:"#667085",fontSize:13},notify:{border:"1px solid #cbd3df",background:"#fff",borderRadius:9,padding:"8px 10px",cursor:"pointer",fontSize:12},notice:{padding:"9px 14px",background:"#fff8db",color:"#755b00",fontSize:13,fontWeight:700,borderBottom:"1px solid #eee1a6"},messages:{height:"calc(100vh - 305px)",minHeight:420,overflowY:"auto",padding:18,background:"#f8fafc"},empty:{color:"#667085",textAlign:"center",paddingTop:80},bubble:{maxWidth:"78%",padding:"10px 13px",borderRadius:12,marginBottom:10,lineHeight:1.45,whiteSpace:"pre-wrap",border:"1px solid #dbe2ec"},sender:{fontSize:11,fontWeight:800,color:"#56657c",marginBottom:3},blocked:{padding:"11px 13px",color:"#a52727",fontWeight:700,background:"#fff4f4",borderRadius:9,width:"100%",textAlign:"center"},time:{fontSize:10,color:"#7a8495",marginTop:4,textAlign:"right"},error:{padding:"10px 14px",color:"#a52727",fontSize:13},composer:{display:"flex",gap:10,padding:14,borderTop:"1px solid #e3e7ee"},input:{flex:1,border:"1px solid #cbd3df",borderRadius:10,padding:"11px 13px",outline:"none"},button:{border:0,borderRadius:10,padding:"11px 18px",background:"#3d78c2",color:"#fff",fontWeight:700}};
