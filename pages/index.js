...
export default function Home(){
 ...
 function start(n){setTest(n);setAgree(false);setSection("essay");setTime(1800);setEssay("");setComp(["","","","",""]);setView("instructions")}
 ...
 if(view==="dashboard")return <><style>{styles}</style><div className="top"><div className="brand">DESCRIPTIVE MASTER</div><div>{name||"Student"}</div></div><div className="wrap"><h1>Descriptive Test</h1><p style={{color:"#697386"}}>IBPS PO • Essay + Comprehension • 25 Marks • 30 Minutes</p><div className="grid">{Array.from({length:10},(_,i)=><div className="card" key={i}><h3>Descriptive Test No {i+1}</h3><div>IBPS PO</div><div className="meta"><span>❔ 2 Qs</span><span>✓ 25 Marks</span><span>◷ 30 Mins</span><span>◉ English</span></div><button className="btn primary" style={{width:"100%"}} onClick={()=>start(i+1)}>→ {i===0?"Start Now":"Start Test"}</button></div>)}</div></div></>;
